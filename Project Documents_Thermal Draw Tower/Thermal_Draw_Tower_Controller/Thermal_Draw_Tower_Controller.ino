...
// define uptake winder speed control pin
...
void setup() {
  ...
  pinMode(neg_limit_read, INPUT_PULLUP); // assigns the pin mode for each limit switch pin
...
void loop() {  
  ...
  if (firstIteration == true) { // sets up the safety mechanism of requiring the user to manually start the feeder by pressing both limit switches at once
    display.clearDisplay(); // Clear the buffer
    display.setTextSize(1);
    display.setTextColor(WHITE);
    display.setCursor(0, 0);

    display.println("PRESS BOTH");
    display.println("LIMIT SWTICHES");
    Serial.println("Press both limit switches simultaneously to start");


    while(pos_limit_check == 0 && neg_limit_check == 0) { // enter a while loop waiting for the limit switches to be triggered
      //pos_limit_switch.loop(); // begin limit switch de-bouncing loops
      //neg_limit_switch.loop();     
      pos_limit_check = pos_limit_switch.getState(); // check limit switch states each loop
      neg_limit_check = neg_limit_switch.getState();
      delay(10); 
      Serial.print("Initial Pos Limit: "); // serial display for troubleshooting purposes
      Serial.print(pos_limit_check);
      Serial.print(" Initial Neg Limit: ");
      Serial.println(neg_limit_check);
    ...
    firstIteration = false; // change first iteration to false after running one time
  }

  // REMOVED CODE PRINTING OUT LIMIT SWITCH POSITION
  // Serial.print("Pos Limit: ");
  // Serial.print(pos_limit_check);
  // Serial.print(" Neg Limit: ");
  // Serial.print(neg_limit_check);
  // Serial.print(" Direction Pin: ");
  // Serial.println(digitalRead(dir_pin));

  // PROCEDURE AFTER BOTH LIMIT SWITCHES HAVE BEEN PRESSED AND RELEASED (USER CONTROLLED)
   ...
   // Serial.println(digitalRead(dir_pin));
    ...
    if (currentMillis - previousMillis >= interval) {
      ...
      */
      ...
      // TASK 3: Read micrometer data
      float fiber_diameter = analogRead(micrometer_pin);
      fiber_diameter *= 4.887585532746823; // analogRead outputs 1023 bits per 10V, then 500 microns per V
      ...
      display.print("Draw ratio:");
      display.println(winder_pot_value/feeder_speed*1000);
    ...
    if (currentMillis - previousMillis >= interval*100) {
      //TASK 5: Output readings to serial monitor every few seconds

      Serial.print(feeder_speed);
      Serial.print(",");
      Serial.print(winder_pot_value);
      Serial.print(",");
      Serial.println(fiber_diameter);
    } 
  }
