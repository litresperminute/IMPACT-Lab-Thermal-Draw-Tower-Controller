...
void loop() {
  ...
  if (pos_limit_check == 0 && neg_limit_check == 0) { // if neither limit switch is triggered (opens on contact) then the program runs as usual.
    ...
    if (currentMillis - previousMillis >= interval) {
      ...
      control_function(winder_pot_value, avg_fiber_diameter, dig_pot, integral_gain); //units of the winder_pot_value going in is in volts needs to be converted
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void control_function(float &winder_pot_value, float fiber_diameter, int &dig_pot, float &int_gain) {
  float setpoint_pot_value = analogRead(winder_pot_value_pin); // Might be computationally better not to initialize this every loop but idk
  setpoint = setpoint_pot_value * 0.2237; // Analog voltage reading to linear winding speed conversion = 0.2237mm/(Volt)
